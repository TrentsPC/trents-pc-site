...
zork.addObject({
  ...
  capacity: 10,
  action: () => {
    if (zork.verb === "TAKE" && zork.directObject === "MAILBOX") {
      zork.tell("It is securely anchored.");
    }
  },
});
...
zork.addRoom({
  ...
  west: () => "FOREST_01",
  east: () => {
    throw "The door is boarded and you can't remove the boards.";
  },
  sw: () => {
    if (zork.globals.WON_FLAG) {
      return "STONE_BARROW";
    }
  },
  in: () => {
  ...
  globals: ["WHITE_HOUSE", "BOARD", "FOREST"],
});
