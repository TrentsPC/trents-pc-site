...
zork.addVerb("OPEN", () => {
  let directObject = zork.objects[zork.directObject!];
  zork.tell(
    "Opening the ",
...
zork.addVerb("RAPE", () => {
  zork.tell("What a (ahem!) strange idea.");
});

zork.addVerb("READ", () => {
  let directObject = zork.objects[zork.directObject!];
  let indirectObject = zork.objects[zork.indirectObject!];
  ...
  if (!directObject?.flags?.includes("READBIT")) {
    return zork.tell(`How does one read a ${directObject.description}?"`);
  }
  zork.tell(directObject.text);
});
...
zork.addVerb("REPENT", () => {
  zork.tell("It could very well be too late!");
});
...
function walk() {
  let direction = zork.directObject || "";
  if (!VALID_DIRECTIONS.includes(direction)) {
    return zork.tell("You can't go that way.");
  }
  ...
  try {
    ...
    if (nextLocation && zork.rooms[nextLocation]) {
      ...
      zork.tell("You can't go that way.");
    }
  } catch (message) {
    return zork.tell(message || "You can't go that way.");
  }
}
zork.addVerb("WALK", walk);
...
function describeRoom() {
  let thisRoom = zork.rooms[zork.here];
  ...
  } else if (thisRoom.longDescription) {
    zork.tell(thisRoom.longDescription);
  }
}
...
function describeObjects() {
  let objects = Object.values(zork.objects).filter((o) => o.in === zork.here);

  objects.forEach((object) => {
    zork.tell(`There is a ${object.description} here.`);
  });
}
...
function printContents(obj: string) {
  const items = Object.values(zork.objects).filter((i) => i.in === obj);
